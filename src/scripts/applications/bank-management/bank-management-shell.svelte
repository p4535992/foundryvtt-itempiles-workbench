  ...
  <DropZone callback={onDropData}>
    ...
    <div class="item-piles-flexrow">
      ...
      <div class="item-piles-flexcol" style="max-height: 319px;">
        ...
        <div class="item-piles-workbench-item-container-list item-piles-bottom-divider" class:item-piles-workbench-centered-flex={!$itemsToAdd.length}>

          {#if !$itemsToAdd.length}

            {#if minVaultSpaces === Infinity}
              <i class="item-piles-workbench-no-items-text" style="flex: 0;">Please select at least one vault</i>
            {:else}
              {#if spaceLeft}
                <i class="item-piles-workbench-no-items-text" style="flex: 0;">Drag and drop items to add them</i>
              {/if}
              <i class="item-piles-workbench-no-items-text" style="flex: 0;">
          ...
          {:else}
            ...
            <div style="margin-bottom: 0.25rem; flex: 0 1 auto;">
              ...
              <i class="item-piles-workbench-no-items-text" style=" margin-top: 0.25rem; align-self: flex-end;">
        ...
        {#if !confirmSending}
          <button type="button" class="item-piles-workbench-button" style="flex: 0 1 auto;"
        ...
        {:else}
          <div class="item-piles-flexrow" style="flex: 0 1 auto;">
            <button type="button" class="item-piles-workbench-button" style="background-color: rgba(219,255,219,0.8);" disabled={!canSendToVaults || buttonHasBeenPressed}
                    on:click={() => sendToVaults()}>
              Confirm
            </button>
            <button type="button" class="item-piles-workbench-button" style="background-color: rgba(255,219,219,0.8);"
